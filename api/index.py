...
APP_CHANGELOG = "Portal switcher: Vanraj + JPPACC in one tool"
...
<title>Password Reset · Multi Portal</title>
<style>
...
.top{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:1rem;gap:1rem}
...
input:focus,select:focus{outline:none;border-color:var(--p)}input.filled{border-color:#22c55e}
...
<div id="gate"><div class="card pin-card"><h1>Access PIN</h1><p class="sub">Multi-portal password tool</p>
...
<p class="ver">v<span id="verL">2.3.0</span> · <a href="#" id="chkUp" style="color:inherit">Check update</a></p></div>
...
<select id="portal">
<option value="vanraj">Vanraj — payment.vaccdharampur.org</option>
<option value="jppacc">JPPACC — student.jppacc.org</option>
</select>
</div>
...
<div id="res"></div>
<div class="note"><b>Portal switcher:</b> Upar se college choose karo.<br>One-Click → token + reset. Success pe Login / Fee buttons.</div>
</div></div>
<script>
const $=id=>document.getElementById(id), PK='tool_unlocked_v1';
const CLIENT_BUILD='2.3.0';
const PORTALS={
  vanraj:{name:'Vanraj College',login:'https://payment.vaccdharampur.org/login',fee:'https://payment.vaccdharampur.org/'},
  jppacc:{name:'JPPACC Student',login:'https://student.jppacc.org/login',fee:'https://student.jppacc.org/'}
};
let pin=sessionStorage.getItem('access_pin')||'', last='', sound=localStorage.getItem('sound')!=='off', serverVer=CLIENT_BUILD;
function portalKey(){return $('portal').value||'vanraj'}
function portal(){return PORTALS[portalKey()]}
function syncPortal(){
  const p=portal();
  $('portalSub').textContent=p.name;
  localStorage.setItem('selected_portal', portalKey());
  $('testBtn').style.display='none';$('openBtn').style.display='none';$('feeBtn').style.display='none';
  $('token').value='';$('token').classList.remove('filled');
}
...
try{const d=await api({action:'find_token',email});if(d.success&&d.token){$('token').value=d.token;$('token').classList.add('filled');prog('OK',100,false);show('Token mil gaya ('+portal().name+')',true)}else show(d.message||'Token nahi mila',false)}catch(e){show(e.message,false)}setTimeout(progHide,600);busy(false)}
...
busy(true);$('testBtn').style.display='none';$('openBtn').style.display='none';$('feeBtn').style.display='none';try{step(2);prog('1/2 Token ('+portal().name+')...',20,false);
const f=await api({action:'find_token',email});if(!f.success||!f.token){prog('Fail',100,false);show(f.message||'No token',false);return}
$('token').value=f.token;$('token').classList.add('filled');prog('2/2 Resetting...',60,false);
...
def reset_password(email, token, password, portal_key="vanraj"):
    ...
    ok = ("password has been reset" in tl or fu.endswith("/home") or "/home" in fu
          or fu.endswith("/login") or "/login" in fu or "/dashboard" in fu or "/muster" in fu)
    if "/password/reset" in fu and ("invalid" in tl or "expired" in tl):
        ok = False
    if ok:
        msg = f"Password reset successful ({p['name']})! Test Login / Fee Portal use karo."
...
def test_login(email, password, portal_key="vanraj"):
    ...
    ok = fu.endswith("/home") or "/home" in fu or "/dashboard" in fu or "/muster" in fu
